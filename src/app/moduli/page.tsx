...
const ModuleCard = memo(({ module, progress }: { module: ModuleMetadata, progress: unknown }) => {
    ...
    const isBeginner = module.difficulty === 'base'
        ...
        <motion.div variants={itemVariants} className="col-span-1 h-full">
            <Link href={`/moduli/${module.id}`} className="block h-full group">
                <Card className={cn(
                    ...
                    isBeginner ? "border-primary/20 hover:border-primary/50" : "border-border hover:border-foreground/20"
                )}>
            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-8">
                ...
                >
                    ...
                    <p className="text-lg text-secondary leading-relaxed">
                        Esplora la library completa. Ogni modulo è progettato per darti indicazioni pratiche, senza farti perdere tempo con dettagli tecnici inutili.
                    </p>
                </motion.div>
                ...
                <div className="space-y-3 pt-6 border-t border-border">
                     ...
                     <div className="flex flex-wrap items-center gap-2">
                        {(['all', 'base', 'intermedia', 'avanzata'] as const).map((level) => (
                            ...
                            >
                                {level === 'all' ? 'Qualsiasi quantita\' di esperienza tecnica' : level}
                            </button>
                        ))}
                    </div>
                </div>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-display font-semibold tracking-tight text-foreground">
                        {filteredModules.length} {filteredModules.length === 1 ? 'Percorso trovato' : 'Percorsi trovati'}
                    </h2>
                </div>
